	...
	Properties
	{
		...
		SubShader
		{
			...
			CGPROGRAM
				...
				float _ShorelineFoamThickness, _ShorelineDefaultThickness;
				...
				void surf(Input IN, inout SurfaceOutput o)
				{
					...
					// Shoreline Foam 연산
					float shorelineMask = 1 - tex2D(_ShorelineFoamTex, IN.uv_ShorelineFoamTex * _ShorelineFoamTiling + float2(_Time.y, _Time.y / 2) * _ShorelineFoamSpeed).r;
					...
					shorelineMask = shorelineMask * (1.0 - shorelineArea);
					...
					float finalMask = smoothstep(animatedThreshold, animatedThreshold+0.6, combinedMask);
					...
					// 최종 색상 혼합 (기존 로직 유지)
					float3 ocean = lerp(_WaterColor.rgb, _ShorelineFoamColor.rgb, shorelineMask);
					o.Emission = ocean.rgb;// saturate(lerp(ocean, floatingFoam, floatingFoam.r));
				}
